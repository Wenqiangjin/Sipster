...
void main(int argc, char **argv)
{
  ...
  printf("Please enter the number of tokens (i.e., K) for this SM.\n");

  int K=10; //set the number of bills 
  scanf("%d", &K); 

  double sm_runtime = 0.0;
  ...
  if(alg_select==1){
    ...
    stop=clock();
    sm_runtime=(double)(stop-start)*1000/CLOCKS_PER_SEC;  
    printf("Computation time of bill issuing phase: %f ms\n",sm_runtime); 
    exit(0);
  }else{
    tt = SM_TokenGen(K,g_rnd_uc,pairing,tt,ecdsa);
  }
